...
namespace StateMachine
{
    public partial class FSMExecutor : IHandle<FSMEvent>, IEnumerable<IFSMNode>
    {
        ...
        public void Stop()
        {
            ...
            Task.Run(() =>
            {
                if (CurrentNode != null && !CurrentNode.Context.IsPaused)
                {
                    ...
                    try
                    {
                        CurrentNode.WaitCurrentTask.Wait();
                    }
                    catch (Exception ex)
                    {
                        e = ex;
        ...
        public bool Restart()
        {
            ...
            InitNodes();
            ExecutorTask = Task.Factory.StartNew(()=>ConsumerTask().Wait());

            return true;
        }

        public bool Restart(FSMNodeContext context)
        {
            if (!StopAndWait())
            { return false; }
        ...
        public bool Restart(IFSMNode node)
        {
            TrackCallname();
            if (!StopAndWait())
            { return false; }
        ...
        public bool Restart(IFSMNode node, FSMNodeContext context)
        {
            if (!StopAndWait())
            { return false; }
